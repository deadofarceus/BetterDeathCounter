...
public class ProgressChartController implements Controller {
    ...
    @Override
    public void init() {
        final Boss boss = player.getCurrentBoss();
        final List<Death> deaths = boss.getDeaths();
        ...
        for (int i = 0; i < size; i++) {
            series.getData().add(new XYChart.Data<>(i+1, 
                boss.getDeaths().get(i).getPercentage()));
        }

        if (settings.getUseCostumPrediction()) {
            myPredArray = boss.getPrediction();
            ...
            for (int i = 0; i < boss.getDeaths().size()+1; i++) {
    ...
    public Parent render() throws IOException {
        ...
        final NumberAxis xaxis = new NumberAxis(0, (int)numDeaths + numDeaths*0.05, 25);  
        ...
        lineChart.setMaxHeight(boss.getSecondPhase() ? 672 : 335);
        lineChart.setMinWidth(1265);
        lineChart.getData().add(series);

        if (settings.getUseCostumPrediction()) {
            if (myPredArray.length != 0) {
                xaxis.setUpperBound((int) ((numDeaths + myPredArray.length)*1.05));//TODO letzter array punkt
            ...
            } else if (boss.getName().equals("Other Monsters or Heights") 
